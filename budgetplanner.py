...
def budgetplanner():
    try:
        budget_select_box = st.selectbox(label="Choose:", options=["Tax Reduction", "Budget Recommendation"])
        st.markdown("---")
        if(budget_select_box == "Tax Reduction"):
            ...
            # Load the finance_data dataset
            finance_data = st.session_state.df

            # Filter the dataset to only include expenses
            expenses_data = finance_data[finance_data['Type'] == 'Expenses']

            # Get the unique components associated with expenses
            expense_components = expenses_data['Component'].unique()

            # Create a sidebar with options for the user to select
            selected_component = st.selectbox('Select the component:', expense_components)

            # Filter the finance_data dataset based on the user's selection
            selected_data = expenses_data[expenses_data['Component'] == selected_component]

            # Create a plot to visualize the data
            # sns.lineplot(x='Year', y='Value', data=selected_data)
            # st.set_option('deprecation.showPyplotGlobalUse', False)
            # st.pyplot()

            fig_budget = px.line(data_frame=selected_data, x="Year", y="Value")
            fig_budget.update_layout(
                        paper_bgcolor='rgba(0,0,0,0)',
                        plot_bgcolor='rgba(0,0,0,0)',   
                    )
                    
            fig_budget.for_each_xaxis(lambda x: x.update(showgrid=False))
            st.plotly_chart(fig_budget, use_container_width=True)


            # Split the data into training and testing sets
            X = selected_data['Year'].values.reshape(-1, 1)
            y = selected_data['Value'].values.reshape(-1, 1)
            ...
            # st.write('Accuracy of the model: ', round(r2, 2))
    except:
        st.error("Please import a csv file")
